...
run_Anchor_012 =0; % 1 means generate pseudoanchor, 2 means use real anchor % 0 means skip
        ...
        channeldistribution=[1,2,3,4,6];% In case of PSEUDOANCHOR,specify,DAPI, A,C,T,G channels
...
p.out_folder='E:\Whole_organoid_realAnchor_v5'; %Folder where output will be 
p.input_folder='K:\Sample 1' %Folder where your CZI files are located
p.rawfiles={
    'Base_1.czi',
    'Base_2.czi',
    'Base_3.czi',
 ...
 p.nchannels = 6 %Number of channels you begin with. NOTE THAT:Pseudoanchor is NOT undestood as a channel. Do not count on it.
...
p.taglist= 'E:\Whole_organoid_realAnchor_v3\taglist_Organoid_fixed_colors.csv' %Location of the taglist used for decoding
p.quality_threshold = 0.7; % Modify the minimum quality
p.background_image = ['K:\iss-analysis-master\lib\omero\bfmatlab\PreprocessingERIK2\OUTPUT ALIGNMENT\Base 1_c1_ALI.tif']; %Select background image if desired
p.I_want_to_plot_on_white_backgound = 1; %1 plots on white background
num_pixels=9 %Minimum number of pixels found on a true spot (filter for Blob identification)
...
t.folder_image = [out_fold,'\Align'];
    ...
    t.tile_size = 2000; %Modify tile size
...
if run_BlobIdentification_YN
    CellProfilerInMatlab_function_3(si.folder,si.PREFIX,si.INTERFIX,si.SUFIX,si.TYPE,ntiles,si.outputdirect,nbases,num_pixels);
end
