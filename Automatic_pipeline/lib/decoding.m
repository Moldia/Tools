...
%detethis in future
seqdat=array2table(seqdata);
writetable(seqdat,[input.output_directory,'\mytable.csv']);
...
make_table_barplot_save(strcat(uTags(idx), ': ', uNames(idx)),...
    cRead(idx), 'Read count before QT',input.output_directory);

% quality bar
category = ones(length(readsNum), 1);
category(ismember(iRead, find(unexpected))) = 0;
category(idxHomo) = -1;
[thres, count] = qualitybar_save(0, 1, category, propsReads(:,6),input.output_directory,'Quality_plot');
drawnow;
toc;
