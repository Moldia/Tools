...
folder='L:\3x3tile\Preprocess\withANCHOR\'
PREFIX='Tile'
INTERFIX='_cyc_'
SUFIX='_CH_'
TYPE='.tif'
...
tform = imregcorr(ATOT(:,:,6,1), ATOT(:,:,6,3));
subplot(4,7,16);
I = ATOT(:,:,6,3);
...
%Image normalization
 for chan=1:6
    for cyc=1:5
   Itop{cyc,chan}=mat2gray(Itop{cyc,chan});
    end
end
 ...
 valuetottot=[];
    ...
    for bases = 1:5
        ...
        for channels = 2:6
            ...
            int=[];
            for pixels = 1:size(X);
               if X(pixels)<2047 & Y(pixels)<2047
                int=[int,fig(X(pixels),Y(pixels))];  
              end
            end  
            value=max(int);
            %if floor(Centroid(i,2))<2047 & floor(Centroid(i,1))<2047
            %value=fig(floor(Centroid(i,1)),floor(Centroid(i,2)));
