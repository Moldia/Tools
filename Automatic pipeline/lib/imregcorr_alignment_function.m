...
function imregcorr_alignment_function(nbases,nchannels, input_folder,dapi,tile_size,ch1,ch2,ch3,ch4,ch5,ch6,...
...
ref_image_prefix=['Base_' num2str(reference_base) '_stitched-'];
ref_image = imread([INPUT_FOLDER_FLO ref_image_prefix num2str(DAPI_channel) '.tif']);
ref_image_re = ref_image;

% This just saves reference base in the new format

for c = 1:number_channels
    output_image_prefix_ref=['Base_' num2str(reference_base) '_aligned-']
    flo = imread([INPUT_FOLDER_REF ref_image_prefix num2str(c) '.tif']);
...
w=chan(si)    
input_image_prefix=['Base_' num2str(w) '_stitched-']
output_image_prefix=['Base_' num2str(w) '_aligned-']
flo_image = imread([INPUT_FOLDER_FLO input_image_prefix num2str(DAPI_channel) '.tif']);
float_fit= flo_image;
% RAM management
clear ref_image;
clear flo_image;
...
for c = 1:number_channels
    ...
    flo = imread([INPUT_FOLDER_REF input_image_prefix num2str(c) '.tif']);
...
if mean(COR(:,1)<0.85)
   error('Correlation of DAPI is under 0.85. Please perform manual alignment before continuing') 
