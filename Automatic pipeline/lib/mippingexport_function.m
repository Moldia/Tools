function  mippingexport_function(directory,rawfiles,outputdirectory,nchan,refro,tilalchi,tilsti)
disp('Entering mipping_export_function');
%Working with 3x3 tiles

image_directory = directory;
image_files = rawfiles;
output_directory = outputdirectory;
nChannels = nchan;
reference_round = refro;
tile_align_channel = tilalchi; % for registration between rounds
tile_stitch_channel = tilsti; %Cy3   % for registration between tiles

% extract tile images from Zeiss files
disp('Mipping tiles from raw files...')
...
for s = 1:numel(files)
    ...
    disp(output_prefix);
    image=bfopen(fullfile(indir,imfile));
    ...
    disp(bfreader.getSizeZ());
    disp(bfreader.getSizeC());
    disp(bfreader.getSizeT());
    disp(bfreader.getSizeX());
    disp(bfreader.getSizeY());
    pause();
            ...
            if ~exist(fullfile(outdir, [output_prefix, '_t', num2str(t) '.tif']), 'file')
                ...
                for c = 1:nChannels
                    if nZstacks == 1
                        ...
                        for z = 1:nZstacks
                            disp('arriveshere')
                            disp(bfreader2.getSizeZ());
                            disp(bfreader2.getSizeC());
                            disp(bfreader2.getSizeT());
                            pause()
